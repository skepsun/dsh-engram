...
function snippetAround(text, needle) {
  ...
  // PRIVACY: these snippets come from RAW session logs (never redacted at the
  // source), so they run through the same deterministic secret redactor as
  // the memory write path before any text reaches the model or the GUI.
  const raw = redactText(t.slice(start, end).replace(/\s*\n+\s*/g, " ").replace(/\s+/g, " ").trim());
