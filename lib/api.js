/**
 * dsh-engram: read-mostly web API for the GUI (host half).
 *
 * A small same-origin JSON route family under `/api/dsh-engram`. Every route
 * wears the loopback-only trust fence (copied from the SSH family): these
 * routes expose the on-disk memory store, so a LAN-exposed dsh web must not
 * serve them. All reads go straight at the opened storage-domain handle
 * (synchronous after the first open); the mutations (archive / delete, and
 * the settings card's PUT /settings) go through the same store methods the
 * agent tools use (settings writes through the `service.settings` seam).
 */

import { uuid, shortId } from "./util.js";
import { collectToolCounts, collectUsageStats } from "./usage.js";
import { SETTINGS_NS } from "./settings.js";

/** Route family prefix (kept in sync with the client half). */
export const API_PREFIX = "/api/dsh-engram";

/** Cap on JSON request bodies (archives/deletes are tiny). */
const MAX_JSON_BODY_BYTES = 32 * 1024;

/** Import dumps carry whole workspaces of memories/tasks — allow up to 32 MiB. */
const MAX_IMPORT_BODY_BYTES = 32 * 1024 * 1024;

/** Loopback literal check plus browser same-origin markers. */
function isLoopbackRequest(request, trustedHosts = []) {
  const address = request.socket?.remoteAddress;
  if (address !== "127.0.0.1" && address !== "::1" && address !== "::ffff:127.0.0.1") return false;
  const host = request.headers.host;
  if (typeof host !== "string") return false;
  let hostUrl;
  try {
    hostUrl = new URL(`http://${host}`);
  } catch {
    return false;
  }
  const hostname = hostUrl.hostname;
  if (hostname === "127.0.0.1" || hostname === "localhost" || hostname === "[::1]") {
    if (request.headers["sec-fetch-site"] === "cross-site") return false;
    const origin = request.headers.origin;
    if (origin === undefined) return true;
    try {
      return new URL(origin).host === hostUrl.host;
    } catch {
      return false;
    }
  }
  // Not loopback: only an explicitly configured trusted hostname (e.g. a
  // cloudflare tunnel the operator authorized) may read the store. This is
  // the opt-in that makes the GUI usable when it is reached through a
  // tunnel; the default stays loopback-only.
  if (trustedHosts.some((h) => typeof h === "string" && h.length > 0 && h.toLowerCase() === hostname.toLowerCase())) {
    if (request.headers["sec-fetch-site"] === "cross-site") return false;
    return true;
  }
  return false;
}

function writeJson(res, status, body) {
  const payload = JSON.stringify(body);
  res.writeHead(status, { "content-type": "application/json; charset=utf-8", "referrer-policy": "no-referrer" });
  res.end(payload);
}

function writeError(res, status, message) {
  writeJson(res, status, { error: message });
}

function readQuery(req) {
  const url = new URL(req.url ?? "/", "http://127.0.0.1");
  return url.searchParams;
}

function readJsonBody(req, maxBytes = MAX_JSON_BODY_BYTES) {
  return new Promise((resolve) => {
    let size = 0;
    const chunks = [];
    req.on("data", (chunk) => {
      size += chunk.length;
      if (size > maxBytes) {
        resolve({ error: "body too large" });
        req.destroy();
        return;
      }
      chunks.push(chunk);
    });
    req.on("end", () => {
      if (size === 0) {
        resolve({ value: {} });
        return;
      }
      try {
        resolve({ value: JSON.parse(Buffer.concat(chunks).toString("utf8")) });
      } catch {
        resolve({ error: "invalid JSON body" });
      }
    });
    req.on("error", () => resolve({ error: "request error" }));
  });
}

/** Coarse token estimate for a [ENGRAM] index block (ASCII ≈ 4 chars/token). */
function estimateTokens(text) {
  if (!text) return 0;
  return Math.max(1, Math.ceil([...text].length / 4));
}

/**
 * Build the route list for one plugin instance.
 * @param service - the engram service ({ config, getDomain, openedDomain, ensureDomain }).
 * @returns the routes to pass to `ctx.webServer.register(route)`.
 */
export function makeEngramRoutes(service) {
  const trustedHosts = Array.isArray(service?.config?.trustedHosts) ? service.config.trustedHosts : [];
  const withDomain = async (res) => {
    const opened = service.openedDomain?.();
    if (opened !== void 0) return opened;
    try {
      return await service.getDomain();
    } catch (error) {
      writeError(res, 503, `engram storage unavailable: ${String(error?.message ?? error)}`);
      return void 0;
    }
  };

  const toolstats = (req, res) => {
    const q = readQuery(req);
    const raw = Number(q.get("days") ?? 14);
    const days = Number.isFinite(raw) && raw >= 1 ? Math.min(90, Math.round(raw)) : 14;
    writeJson(res, 200, collectToolCounts({ root: service.statsRoot, days }));
  };

  const readConfig = (_req, res) => {
    const c = service.config ?? {};
    const payload = {
      autoCapture: c.autoCapture,
      sessionSearch: c.sessionSearch,
      autoCapturePerSession: c.autoCapturePerSession,
      indexMaxLines: c.indexMaxLines,
      indexMaxChars: c.indexMaxChars,
      minIndexSignal: c.minIndexSignal,
      promoteHits: c.promoteHits,
      expireDays: c.expireDays,
      maxMemoriesPerWorkspace: c.maxMemoriesPerWorkspace,
    };
    writeJson(res, 200, payload);
  };

  const overview = async (req, res) => {
    const domain = await withDomain(res);
    if (domain === void 0) return;
    const summary = domain.summarize();
    // [ENGRAM] index estimate: recompute the exact block cost per workspace.
    const indexes = {};
    for (const ws of Object.keys(summary.workspaces)) {
      const block = service.renderIndexBlock?.(ws) ?? "";
      indexes[ws] = { chars: [...block].length, tokens: estimateTokens(block), lines: block.split("\n").length - 1 };
    }
    writeJson(res, 200, {
      ...summary,
      indexes,
      captures: service.captureStats ?? { total: 0, git: 0, file: 0, error: 0 },
      gc: service.gcStats ?? { lastRun: 0, archivedMemories: 0, archivedTasks: 0, removedLinks: 0 },
      compaction: service.compactionStatus ?? { host: "unknown", web: { action: "unknown", presets: [] } },
      config: service.config ?? {},
      anonymous: {
        version: 2,
        note: "counts exclude archived/expired entries; tasks exclude stable.",
      },
    });
    void req;
  };

  const memories = async (req, res) => {
    const domain = await withDomain(res);
    if (domain === void 0) return;
    const params = readQuery(req);
    const items = domain.searchMemories({
      workspace: params.get("workspace") || void 0,
      q: params.get("q") || void 0,
      kind: params.get("kind") || void 0,
      status: params.get("status") || void 0,
      limit: Number(params.get("limit") ?? 100),
    });
    writeJson(res, 200, { items });
  };

  // Goal-domain sediment: memories auto-captured from `goal/change` events
  // (tag "goal"). A compact read surface for the GUI, backed by the same
  // searchMemory view as /memories.
  const goals = async (req, res) => {
    const domain = await withDomain(res);
    if (domain === void 0) return;
    const params = readQuery(req);
    const limit = Number(params.get("limit") ?? 50);
    const items = domain.searchMemories({
      workspace: params.get("workspace") || void 0,
      tag: "goal",
      status: "active",
      limit,
    }).map((m) => ({ id: m.id, kind: m.kind, text: m.text, createdAt: m.createdAt, workspace: m.workspace }));
    writeJson(res, 200, { items });
  };

  const tasks = async (req, res) => {
    const domain = await withDomain(res);
    if (domain === void 0) return;
    const params = readQuery(req);
    const workspace = params.get("workspace");
    if (!workspace) {
      writeError(res, 400, "workspace query parameter is required");
      return;
    }
    const includeStable = params.get("includeStable") === "1";
    writeJson(res, 200, { items: domain.listTasks(workspace, { includeStable }) });
  };

  const links = async (req, res) => {
    const domain = await withDomain(res);
    if (domain === void 0) return;
    const params = readQuery(req);
    const workspace = params.get("workspace");
    if (!workspace) {
      writeError(res, 400, "workspace query parameter is required");
      return;
    }
    writeJson(res, 200, { items: domain.allLinks(workspace) });
  };

  const entities = async (req, res) => {
    const domain = await withDomain(res);
    if (domain === void 0) return;
    const params = readQuery(req);
    const workspace = params.get("workspace");
    if (!workspace) {
      writeError(res, 400, "workspace query parameter is required");
      return;
    }
    writeJson(res, 200, { items: domain.listEntities(workspace) });
  };

  const archive = async (req, res) => {
    const domain = await withDomain(res);
    if (domain === void 0) return;
    const body = await readJsonBody(req);
    if (body.error) return writeError(res, 400, body.error);
    const { id, workspace } = body.value ?? {};
    if (typeof id !== "string" || typeof workspace !== "string") {
      return writeError(res, 400, "id and workspace are required");
    }
    try {
      const ok = await domain.archiveMemory(workspace, id);
      if (!ok) return writeError(res, 404, "memory not found or already archived");
      writeJson(res, 200, { ok: true });
    } catch (error) {
      writeError(res, 500, String(error?.message ?? error));
    }
  };

  const remove = async (req, res) => {
    const domain = await withDomain(res);
    if (domain === void 0) return;
    const body = await readJsonBody(req);
    if (body.error) return writeError(res, 400, body.error);
    const { id, workspace } = body.value ?? {};
    if (typeof id !== "string" || typeof workspace !== "string") {
      return writeError(res, 400, "id and workspace are required");
    }
    try {
      const ok = await domain.deleteMemory(workspace, id);
      if (!ok) return writeError(res, 404, "memory not found");
      writeJson(res, 200, { ok: true });
    } catch (error) {
      writeError(res, 500, String(error?.message ?? error));
    }
  };

  const gcRun = async (req, res) => {
    const domain = await withDomain(res);
    if (domain === void 0) return;
    const body = await readJsonBody(req);
    if (body.error) return writeError(res, 400, body.error);
    const { workspace, dryRun } = body.value ?? {};
    // Workspace optional -> sweep every workspace (same semantics as the tool).
    if (workspace !== void 0 && typeof workspace !== "string") {
      return writeError(res, 400, "workspace must be a string when provided");
    }
    try {
      const report = await domain.gc(
        workspace === void 0 || workspace.length === 0 ? void 0 : workspace,
        service.config ?? {},
        { dryRun: dryRun === true },
      );
      if (dryRun !== true) {
        service.gcStats ??= { lastRun: 0, archivedMemories: 0, archivedTasks: 0, removedLinks: 0 };
        service.gcStats.lastRun = Date.now();
        service.gcStats.archivedMemories += report.archivedMemories.length;
        service.gcStats.archivedTasks += report.archivedTasks.length;
        service.gcStats.removedLinks += report.removedLinks.length;
      }
      writeJson(res, 200, { report });
    } catch (error) {
      writeError(res, 500, String(error?.message ?? error));
    }
  };

  /**
   * Data contract — dump every row (memories/tasks/links/entities, archived
   * included) of one workspace (`?workspace=`) or of every workspace, in the
   * machine-readable `dsh-engram/export` v1 format that `POST /import`
   * round-trips. Export is the stable, format-versioned exit door: the memory
   * corpus never depends on engram's internal tables, and a thin adapter
   * (another harness, MCP, a plain backup script) only needs this shape.
   */
  const exportData = async (req, res) => {
    const domain = await withDomain(res);
    if (domain === void 0) return;
    const workspace = readQuery(req).get("workspace") || void 0;
    try {
      writeJson(res, 200, domain.exportAll(workspace));
    } catch (error) {
      writeError(res, 500, String(error?.message ?? error));
    }
  };

  /** Data contract — restore a `dsh-engram/export` dump (see store.importAll:
   *  merge = idempotent add-missing, replace = wipe workspace then restore;
   *  `dryRun` plans without writing; `confirm: "restore"` is required for the
   *  destructive replace mode). Memory text passes the same secret redactor as
   *  every other write path. */
  const importData = async (req, res) => {
    const domain = await withDomain(res);
    if (domain === void 0) return;
    const body = await readJsonBody(req, MAX_IMPORT_BODY_BYTES);
    if (body.error) return writeError(res, 400, body.error);
    const { payload, mode, dryRun, workspace, confirm } = body.value ?? {};
    if (mode !== "merge" && mode !== "replace") {
      return writeError(res, 400, "mode must be 'merge' or 'replace'");
    }
    if (mode === "replace" && confirm !== "restore") {
      return writeError(res, 400, "replace mode is destructive — pass confirm: \"restore\" to proceed");
    }
    try {
      const report = await domain.importAll(payload, {
        mode: mode ?? "merge",
        dryRun: dryRun === true,
        workspace: typeof workspace === "string" && workspace.length > 0 ? workspace : void 0,
        config: service.config ?? {},
      });
      writeJson(res, 200, report);
    } catch (error) {
      const status = error instanceof Error && error.code === "ENGRAM_INVALID_ARGS" ? 400 : 500;
      writeError(res, status, String(error?.message ?? error));
    }
  };

  const guard = (req, res, method, handler) => {
    if ((req.method ?? "GET").toUpperCase() !== method) {
      writeError(res, 405, "method not allowed");
      return;
    }
    if (!isLoopbackRequest(req, trustedHosts)) {
      writeError(res, 403, "loopback-only");
      return;
    }
    return handler(req, res);
  };

  /** GUI task creation — mirrors esr_task (id + state defaults), no session. */
  const createTask = async (req, res) => {
    const domain = await withDomain(res);
    if (domain === void 0) return;
    const body = await readJsonBody(req);
    if (body.error) return writeError(res, 400, body.error);
    const { workspace, name, description } = body.value ?? {};
    if (typeof workspace !== "string" || workspace.length === 0) return writeError(res, 400, "workspace is required");
    if (typeof name !== "string" || name.trim().length === 0) return writeError(res, 400, "name is required");
    const max = service.config?.maxTasksPerWorkspace ?? 10;
    if (domain.activeTaskCount(workspace) >= max) {
      return writeError(res, 409, `workspace already holds ${max} active tasks; close some first`);
    }
    const now = Date.now();
    const task = {
      id: `tsk_${shortId(uuid())}`,
      workspace,
      name: name.trim().slice(0, 200),
      description: typeof description === "string" ? description.trim().slice(0, 2000) : "",
      state: "active",
      artifact: null,
      evaluation: null,
      memoryRefs: [],
      sessionId: "gui",
      createdAt: now,
      updatedAt: now,
      stateChangedAt: 0,
      deps: [],
      assignee: null,
      claimedAt: null,
    };
    await domain.putTask(task);
    writeJson(res, 200, { task });
  };

  /** GUI task close — mirrors esr_close's evidence gates without a session. */
  const closeTask = async (req, res) => {
    const domain = await withDomain(res);
    if (domain === void 0) return;
    const body = await readJsonBody(req);
    if (body.error) return writeError(res, 400, body.error);
    const { workspace, id, artifact, evaluation, memory_refs } = body.value ?? {};
    if (typeof workspace !== "string" || workspace.length === 0 || typeof id !== "string" || id.length === 0) {
      return writeError(res, 400, "workspace and id are required");
    }
    const task = domain.getTask(workspace, id);
    if (task === void 0) return writeError(res, 404, "task not found");
    const str = (v) => (typeof v === "string" && v.trim().length > 0 ? v.trim() : undefined);
    const refs = Array.isArray(memory_refs)
      ? memory_refs.filter((x) => typeof x === "string" && x.trim().length > 0).map((x) => x.trim())
      : undefined;
    const next = {
      ...task,
      artifact: str(artifact) ?? task.artifact ?? null,
      evaluation: str(evaluation) ?? task.evaluation ?? null,
      memoryRefs: refs ?? task.memoryRefs ?? [],
      updatedAt: Date.now(),
    };
    const gate = domain.evidenceGate(workspace, next, service.config ?? {});
    const gaps = gate.gaps;
    if (gaps.length > 0) {
      await domain.putTask(next);
      writeJson(res, 200, { ok: true, state: "active", gaps, artifactReason: gate.artifactReason });
      return;
    }
    const closed = domain.compactOnClose?.({ ...next, state: "stable", stateChangedAt: Date.now() }) ?? { ...next, state: "stable", stateChangedAt: Date.now() };
    await domain.putTask(closed);
    writeJson(res, 200, { ok: true, state: "stable", task: closed });
  };

  /** Exact [ENGRAM] + [ESR] injection blocks for one workspace (GUI preview). */
  const preview = async (req, res) => {
    const domain = await withDomain(res);
    if (domain === void 0) return;
    const params = readQuery(req);
    const workspace = params.get("workspace");
    if (!workspace) {
      writeError(res, 400, "workspace query parameter is required");
      return;
    }
    const engram = service.renderIndexBlock?.(workspace) ?? "";
    const esrBlock = service.renderEsrBlock?.(workspace) ?? "";
    const counts = domain.workspaceEntries(workspace);
    writeJson(res, 200, {
      workspace,
      engram,
      esr: esrBlock,
      meta: {
        engram: {
          chars: [...engram].length,
          tokens: estimateTokens(engram),
          lines: engram === "" ? 0 : engram.split("\n").length,
        },
        esr: {
          chars: [...esrBlock].length,
          tokens: estimateTokens(esrBlock),
          lines: esrBlock === "" ? 0 : esrBlock.split("\n").length,
        },
        counts: { memories: counts.memories, tasks: counts.tasks, links: counts.links, nodes: counts.nodes },
      },
    });
    void req;
  };

  /** Real agent-behaviour observability: usage rollup + ratios, computed on
   *  demand from the session log stream (assessment P3 — no per-call writes). */
  const stats = async (req, res) => {
    const params = readQuery(req);
    const workspace = params.get("workspace") ?? void 0;
    const collected = collectUsageStats({ root: service.statsRoot });
    const allRows = collected.rows;
    const rows = workspace === void 0 ? allRows : allRows.filter((r) => r.workspace === workspace);
    const totals = { counts: {}, failures: 0, recall: {} };
    for (const r of rows) {
      for (const [k, v] of Object.entries(r.counts ?? {})) totals.counts[k] = (totals.counts[k] ?? 0) + v;
      totals.failures += r.failures ?? 0;
      for (const [k, v] of Object.entries(r.recall ?? {})) totals.recall[k] = (totals.recall[k] ?? 0) + v;
    }
    const ENGRAM = ["engram_store", "engram_recall", "engram_detail", "loom_store", "loom_recall", "loom_detail"];
    const ESR = ["esr_task", "esr_node", "esr_close", "esr_link", "esr_gc"];
    const sum = (keys) => keys.reduce((acc, k) => acc + (totals.counts[k] ?? 0), 0);
    const memCalls = sum(ENGRAM);
    const esrCalls = sum(ESR);
    const calls = memCalls + esrCalls;
    const queries = totals.recall.queries ?? 0;
    const withHits = totals.recall.withHits ?? 0;
    const hitsTotal = totals.recall.hitsTotal ?? 0;
    const detailFollows = totals.recall.detailFollows ?? 0;
    writeJson(res, 200, {
      workspace: workspace ?? null,
      byDay: rows
        .slice()
        .sort((a, b) => (a.day < b.day ? -1 : a.day > b.day ? 1 : 0))
        .slice(-14)
        .reverse(),
      totals,
      ratios: {
        calls,
        esrCalls,
        memCalls,
        esrRatio: calls > 0 ? +(esrCalls / calls).toFixed(4) : null,
        recallHitRate: queries > 0 ? +(withHits / queries).toFixed(4) : null,
        recallHitsPerQuery: queries > 0 ? +(hitsTotal / queries).toFixed(3) : null,
        detailFollowRate: queries > 0 ? +(detailFollows / queries).toFixed(4) : null,
      },
    });
  };

  const triggerstats = (_req, res) => {
    const q = readQuery(_req);
    const ws = q.get("workspace");
    const stats = service.recorder?.conversionStats?.(ws && ws.length > 0 ? ws : void 0) ?? null;
    writeJson(res, 200, { stats });
  };

  const model = async (req, res) => {
    const domain = await withDomain(res);
    if (domain === void 0) return;
    const ws = readQuery(req).get("workspace") ?? "";
    try {
      const { getModel } = await import("./mental.js");
      const m = await getModel(domain, ws);
      writeJson(res, 200, { model: m });
    } catch (err) {
      writeJson(res, 500, { error: err instanceof Error ? err.message : String(err) });
    }
  };

  const observations = async (req, res) => {
    const domain = await withDomain(res);
    if (domain === void 0) return;
    const ws = readQuery(req).get("workspace") ?? "";
    const list = domain.listObservations?.(ws) ?? [];
    writeJson(res, 200, { items: list });
  };

  /** Settings card transport — read the `dsh-engram` namespace through the
   *  host settings service (see settings.js for the normalized `service.settings`
   *  seam). Same-origin fetch, like every other route in this family; this is
   *  what lets the web config card be DSH-generation-agnostic (no
   *  connection/remote RPC), at the cost of the same loopback/trustedHosts
   *  fence the memory viewer already wears. */
  const getSettings = (_req, res) => {
    const settings = service.settings;
    if (!settings) {
      writeError(res, 503, "settings service not available");
      return;
    }
    try {
      // dsh-settings `describe()` returns a BARE array of namespace descriptors
      // (in registration order); older adapters may wrap it as {namespaces:[...]}.
      // Normalize both onto the HTTP contract {namespaces:[...]} the client uses.
      const payload = settings.describe();
      const namespaces = Array.isArray(payload) ? payload : (payload?.namespaces ?? []);
      const view = namespaces.find((n) => n && n.ns === SETTINGS_NS);
      writeJson(res, 200, view ? { namespaces: [view] } : { namespaces: [] });
    } catch (error) {
      writeError(res, 500, String(error?.message ?? error));
    }
  };

  /** Settings card transport — mutate the `dsh-engram` namespace. Accepts
   *  either path ops (`ops: [{op:"set"|"unset", path:[...], value?}]`, the
   *  exact shape the card used through the RPC) or a plain partial patch
   *  (`patch: object`). The provider validates and persists through the same
   *  document the GUI card always wrote to. */
  const putSettings = async (req, res) => {
    const settings = service.settings;
    if (!settings) {
      writeError(res, 503, "settings service not available");
      return;
    }
    const body = await readJsonBody(req);
    if (body.error) return writeError(res, 400, body.error);
    const { ops, patch, expectedRevision } = body.value ?? {};
    if (!Array.isArray(ops) && !(patch !== null && typeof patch === "object")) {
      return writeError(res, 400, "body must provide ops (array of {op,path[,value]}) or patch (object)");
    }
    try {
      if (Array.isArray(ops)) {
        await settings.mutate(SETTINGS_NS, ops, expectedRevision);
      } else {
        await settings.update(SETTINGS_NS, patch, expectedRevision);
      }
      writeJson(res, 200, { ok: true });
    } catch (error) {
      if (error?.code === "SETTINGS_CONFLICT" || error?.name === "SettingsConflictError") {
        writeJson(res, 409, {
          error: {
            code: "SETTINGS_CONFLICT",
            message: String(error?.message ?? "settings moved since read"),
            expected: error?.expected,
            actual: error?.actual,
          },
        });
        return;
      }
      writeError(res, 400, String(error?.message ?? error));
    }
  };

  const make = (method, path, handler) => ({
    kind: "exact",
    path: `${API_PREFIX}${path}`,
    method,
    handler: (req, res) => guard(req, res, method, handler),
  });

  /** One exact route serving several methods on the same path. The host
      webServer's exact table is keyed by pathname only — two separate routes
      on `/tasks` (GET + POST) collide and the second register() throws
      "duplicate route", which the mount silently skips. So a shared path must
      be a single route that dispatches on req.method itself. */
  const makeUnion = (path, byMethod) => {
    const methods = Object.keys(byMethod);
    return {
      kind: "exact",
      path: `${API_PREFIX}${path}`,
      method: methods,
      handler: (req, res) => {
        const m = String(req.method ?? "GET").toUpperCase();
        const inner = byMethod[m];
        if (inner === void 0) {
          writeError(res, 405, "method not allowed");
          return;
        }
        return guard(req, res, m, inner);
      },
    };
  };

  return [
    make("GET", "/overview", overview),
    make("GET", "/memories", memories),
    make("GET", "/goals", goals),
    makeUnion("/tasks", { GET: tasks, POST: createTask }),
    make("GET", "/links", links),
    make("GET", "/nodes", entities),
    make("GET", "/stats", stats),
    make("GET", "/toolstats", toolstats),
    make("GET", "/triggerstats", triggerstats),
    make("GET", "/observations", observations),
    make("GET", "/model", model),
    make("GET", "/preview", preview),
    make("GET", "/config", readConfig),
    makeUnion("/settings", { GET: getSettings, PUT: putSettings }),
    make("POST", "/memories/archive", archive),
    make("POST", "/memories/delete", remove),
    make("POST", "/tasks/close", closeTask),
    make("POST", "/gc", gcRun),
    make("GET", "/export", exportData),
    make("POST", "/import", importData),
  ];
}
