/**
 * dsh-loom: real-data agent-behaviour observability.
 *
 * Every loom_* / esr_* tool call made by the model is recorded into the
 * store's per-(workspace, day) `usage` rollup: counts per tool, failures,
 * and recall mechanics. The GUI (`/api/dsh-loom/stats`) turns that into the
...
/**
 * Count recalled memory rows from a loom_recall output string. The tool
...
/** Session-local drill-through window: a loom_detail within N events of a hit recall counts as a follow-up. */
...
export function makeUsageTracker() {
  ...
  return {
    ...
    async record(service, exec, name, opts) {
      ...
      } else if (name === "loom_detail" && opts.ok && recallFresh.has(sessionId)) {
      ...
      try {
        ...
        service?.log?.warn?.(`loom usage record failed: ${String(error)}`);
      }
    },
  };
}