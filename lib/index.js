 ...
 *   - six tight tools: engram_store / engram_recall / engram_detail / esr_task /
 *     esr_close / esr_link
...
export function apply(ctx, config) {
  ...
  const service = {
    ...
    /** Recompute the exact [ESR] block for a workspace (GUI inject preview). */
    renderEsrBlock(workspace) {
      if (opened === void 0) return "";
      try {
        return renderEsr(opened, workspace, resolved, { recorder });
      } catch {
        return "";
      }
    },
