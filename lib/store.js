...
import { hashText, byRecency, uuid, now, truncate as truncateText, bm25Rank, tokenize as tokenizeLocal } from "./util.js";
  ...
  return {
    ...
    async storeMemory(entry, config) {
      ...
      if ((entry.kind ?? "fact") === "error") {
        ...
        for (const prev of allMemories(entry.workspace)) {
          ...
          if (!repeatOverlap(prev.text, text)) continue;
          const revived = { ...prev, updatedAt: now(), hits: (prev.hits ?? 0) + 1 };
    ...
    recall(workspace, query, limit = 20) {
      ...
      if (query !== void 0 && String(query).trim().length > 0) {
        return bm25Rank(active, String(query).trim(), bounded);
      }
