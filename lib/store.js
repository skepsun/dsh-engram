...
export async function openLoomDomain(facility) {
  ...
  return {
    ...
    searchMemories({ workspace, q, kind, status, limit = 100 } = {}) {
      ...
      pool = pool.filter(notExpiredNow);
    ...
    summarize() {
      ...
      for (const [, m] of memories.entries()) {
        ...
        workspaces[ws] ??= { memories: 0, tasks: 0, links: 0 };
        if (m.status === "active") {
      ...
      for (const [, t] of tasks.entries()) {
        workspaces[t.workspace] ??= { memories: 0, tasks: 0, links: 0 };
        if (t.state !== "stable") {
      ...
      for (const [, l] of links.entries()) {
        workspaces[l.workspace] ??= { memories: 0, tasks: 0, links: 0 };
        workspaces[l.workspace].links += 1;
        linkCount += 1;
      }
      return { workspaces, kinds, totals: { memories: memoryCount, tasks: taskCount, links: linkCount } };
    },
