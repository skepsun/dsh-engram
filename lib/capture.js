  ...
  return (exec, result) => {
    try {
      ...
      if (git !== null) {
        ...
        if (testFail !== null) {
          entry = { kind: "error", text: testFail, signal: TEST_FAILURE_SIGNAL, tags: ["error", "test", "auto-captured"], filePath: null };
        } else {
          const msg = errorHead(result);
          if (msg !== null) {
            entry = { kind: "error", text: `${name} failed: ${msg}`, signal: 0.25, tags: ["error", "auto-captured"], filePath: errPath || null };
          }
        }
