...
export function makeTriggerRecorder(config = {}) {
  ...
  // Injectable clock (deterministic unit tests): default to the wall clock.
  ...
  function bump(workspace, key) {
    ...
    if (buckets.size > BALANCE_DAYS + 2) {
      const oldest = dayKey(now() - BALANCE_DAYS * DAY_MS);
  ...
  function handle(exec, result) {
    ...
    try {
      ...
      if (ESR_TOOLS.includes(name)) attributeConversion(workspace, now());
      ...
      todos.set(workspace, { sessionId, count: names.length, names, updatedAt: now() });
  ...
  function prunePending() {
    const cutoff = now() - CONVERSION_WINDOW_MS;
  ...
  function emitHint(kind, workspace, sessionId) {
    ...
    pending.push({ kind, ws: workspace, session: sessionId, shownAt: now() });
    prunePending();
  }
  ...
  function staleHint(sessionId, workspace, staleTasks) {
    ...
    for (const t of staleTasks) {
      ...
      staleSeen.add(key);
      const at = t.updatedAt ?? t.createdAt ?? now();
      const days = Math.max(1, Math.round((now() - at) / DAY_MS));
