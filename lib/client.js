...
function buildTasksMarkdown(tasks) {
  ...
  const rows = tasks.slice().sort((a, b) => a.state.localeCompare(b.state) || (a.createdAt ?? 0) - (b.createdAt ?? 0)).map((t) => {
    ...
    return `| ${t.state} | ${esc(t.name)} | ${esc(t.workspace.replace(/^.*[\\/]/, ""))} | ${esc(taskGaps3(t).join(", ") || "\u2014")} | ${ev} | ${fmtDate3(t.createdAt ?? 0)} |`;
  });
    ...
    try {
      ...
      const entries = await Promise.all(
        wsList.map(async (w) => {
          ...
          const res = shouldLoad ? await Promise.all([api.tasks(w, true), api.nodes(w), api.links(w)]) : null;
          return [w, res ? { tasks: res[0].items, nodes: res[1].items, links: res[2].items } : null];
        })
      );
      setTasksByWs((prev) => {
        const next = { ...prev };
