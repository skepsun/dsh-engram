...
${hint}` : ""}`;
      ...
      registerTool({
        name: "esr_model",
        description: "Precomputed mental model — the workspace's standing answer: active/ready/closed task counts, entity graph size, top evidence-grounded beliefs (observations) and open risks. Zero LLM: reads the cached aggregate and recomputes only when data changed (any esr_* write, or a >10min staleness safety net). Read this before scattering recall queries when you need the current state of the workspace.",
        parameters: {
          type: "object",
          properties: {
        ...
        timeoutMs: 20000,
        execute: async (raw, exec) => {
          const args = parse(z.object({ ws: z.string().optional() }), raw, "esr_model");
          const caller = callerOf(exec);
          const domain = await requireDomain(service);
          const ws = args.ws ?? caller.cwd;
          const { getModel } = await import("./mental.js");
          const model = await getModel(domain, ws);
          const mins = Math.max(0, Math.floor((Date.now() - model.generated_at) / 60000));
          return `${model.content}\n\n(生成于 ${mins} 分钟前 · ${model.fresh ? "本次重算" : "缓存命中"} · ws=${ws})`;
        },
      }),
    ];
    return () => {
