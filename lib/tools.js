...
export function registerTools(ctx, service) {
  return ctx.effect(() => {
    ...
    const disposers = [
      ...
      registerTool({
        name: "engram_store",
        description: "Store one memory: kind=(decision|error|procedure|fact|insight|handoff|task), text, optional entity anchor, optional supersedes/contradicts memory ids (stale truth vs conflicting claim). Exact-duplicate text refreshes instead of adding; auto-capture already handles git/file/error events — use this for anything it misses.",
        parameters: {
          type: "object",
          properties: {
...
${hint}` : ""}`;
      ...
      registerTool({
        name: "esr_status",
        description:
          "Pull the live ESR state (deterministic snapshot + revision) plus decision-point hints (promote / root-cause / close / stale / escalate). Pass since_revision to skip unchanged state — returns a short response when nothing changed. The [ESR] block in the prompt is a session-start snapshot that never auto-refreshes; call this for the current truth.",
        parameters: {
          type: "object",
          properties: {
            workspace: { type: "string", description: "Override the workspace key (default: current session cwd)" },
            since_revision: { type: "string", description: "Last-seen ESR revision (from a previous esr_status response); unchanged state returns a short response" },
          },
        },
        output: TEXT_OUTPUT,
        timeoutMs: 20000,
