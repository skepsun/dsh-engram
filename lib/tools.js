...
export function registerTools(ctx, service) {
  return ctx.effect(() => {
    ...
    const disposers = [
      ...
      registerTool({
        name: "esr_task",
        description: "Create/update an engineering task (draft→active). A task is only 'done' with evidence — finish via esr_close (artifact + evaluation + memory_ref). Set id to update an existing task.",
        parameters: {
          type: "object",
          properties: {
