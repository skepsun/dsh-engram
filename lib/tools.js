...
function memoryLine(m, verbosity = "short") {
  ...
  if (verbosity === "full") {
    ...
    return [`# ${m.id} [${m.kind}]`, escapeLt(m.text), `tags: ${(m.tags ?? []).join(", ")}${tags}`, `signal ${m.signal} · hits ${m.hits ?? 0} · ${fmtDate(m.createdAt)} · session ${m.sessionId}#${m.seq} · ${access}`, ""].join("\n");
  }
  ...
  return ctx.effect(() => {
    ...
    const disposers = [
      ...
      registerTool({
        ...
        timeoutMs: 30000,
        execute: async (raw, exec) => {
          const args = parse(z.object({ query: z.string().optional(), entity: z.string().optional(), limit: z.number().int().min(1).max(50).optional(), max_per_session: z.number().int().min(1).max(10).optional(), search_sessions: z.boolean().optional() }), raw, "engram_recall");
          const caller = callerOf(exec);
          const domain = await requireDomain(service);
          let lines = [];
          let items = [];
...
${hint}` : ""}`;
      ...
      registerTool({
        name: "esr_dep",
        description: "Add a dependency edge between two tasks (Beads-inspired mini graph). kind: blocks = this task waits on dep_id (not claimable until dep closes), parent-of = dep_id is this task's child, relates-to = no blocking. Refuses self-deps and directed cycles. deps surface in the ESR board as lock markers.",
        parameters: {
          type: "object",
          properties: {
        ...
        execute: async (raw, exec) => {
          ...
          const caller = callerOf(exec);
          const domain = await requireDomain(service);
          const t = await domain.addDep(caller.cwd, resolveTaskId(domain, caller.cwd, args.task_id), resolveTaskId(domain, caller.cwd, args.dep_id), args.kind ?? "blocks");
          await domain.markModelDirty?.(caller.cwd).catch?.(() => null);
          return `dep added: ${t.id} --${args.kind ?? "blocks"}--> ${args.dep_id} (total ${(t.deps ?? []).length} edges)`;
        },
      }),
      ...
      registerTool({
        name: "esr_claim",
        description: "Atomically claim a task: sets assignee + claimedAt (draft becomes active). Anti-yank fence: refuses when another agent already claimed it, unless force:true. Use esr_ready to find claimable work.",
        parameters: {
          type: "object",
          properties: {
            task_id: { type: "string", description: "Task id from [ESR] or esr_task" },
      ...
      registerTool({
        name: "esr_unclaim",
        description: "Release ownership of a claimed task (assignee back to null). Anti-yank fence: a non-holder needs force:true. Inverse of esr_claim.",
        parameters: {
          type: "object",
          properties: {
            task_id: { type: "string", description: "Task id from [ESR] or esr_task" },
      ...
      registerTool({
        name: "esr_ready",
        description: "List claimable work: tasks with no open blocker and nobody claimed (Beads-style ready queue). One line per task so an agent can pick the next thing to claim.",
        parameters: {
          type: "object",
          properties: {
...
${hint}` : ""}`;
      ...
      registerTool({
        name: "esr_model",
        description: "Precomputed mental model — the workspace's standing answer: active/ready/closed task counts, entity graph size, top evidence-grounded beliefs (observations) and open risks. Zero LLM: reads the cached aggregate and recomputes only when data changed (any esr_* write, or a >10min staleness safety net). Prime-style budget: mode=brief returns a ~50-token one-liner for session-start injection; max_chars trims the output at a whole-char boundary (tail condensed). Read this before scattering recall queries when you need the current state of the workspace.",
        parameters: {
          type: "object",
          properties: {
