...
function memoryLine(m, verbosity = "short", rel) {
  ...
  if (verbosity === "full") {
    ...
    return [`# ${m.id} [${m.kind}]`, escapeLt(m.text), `tags: ${(m.tags ?? []).join(", ")}${tags}`, `signal ${m.signal} · hits ${m.hits ?? 0} · ${fmtDate(m.createdAt)} · session ${m.sessionId}#${m.seq} · ${access}${pathLine}${relLine ? ` · ${relLine}` : ""}`, ""].join("\n");
  }
  const date = fmtDate(m.createdAt ?? m.updatedAt);
  const path = m.filePath ? ` · ${truncate(m.filePath, 48)}` : "";
  return `- ${m.id.slice(0, 8)} [${m.kind}] ${date} ${escapeLt(truncate(m.text, 200))}${tags}${m.hits > 0 ? ` · ×${m.hits}` : ""}${path}${relTail}`;
}
 ...
 */
function groupKindLines(items, rel) {
  ...
  const emit = (kind, group) => {
    ...
    for (const m of group) lines.push(memoryLine(m, "short", rel));
  };
    ...
    const disposers = [
      ...
      registerTool({
        ...
        execute: async (raw, exec) => {
          const args = parse(z.object({ text: z.string().min(1).max(2000), kind: z.enum(KINDS).optional(), tags: z.array(z.string()).optional(), entity: z.string().optional(), file_path: z.string().optional(), supersedes: z.string().optional(), contradicts: z.string().optional() }), raw, "engram_store");
      ...
      registerTool({
        ...
        execute: async (raw, exec) => {
          ...
          if (args.entity !== void 0 && args.entity.trim().length > 0) {
            ...
            lines.push(...(args.group_by_kind === false ? items.map((m) => memoryLine(m, "short", rel)) : groupKindLines(items, rel)));
            await Promise.all(items.map((m) => domain.touchMemory(caller.cwd, m.id).catch(() => null)));
          } else {
          ...
          if (wantSessions) {
            ...
            const extra = await searchSessions(service, caller, q, exec.signal);
            if (extra.length > 0) {
