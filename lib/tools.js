 ...
 *
 * Fourteen small, tight-schema tools (schemas cost tokens in every request):
 *   engram_store / engram_recall / engram_detail    — the engram memory surface
 *   esr_task / esr_close / esr_link / esr_dep / esr_claim / esr_unclaim /
 *   esr_node / esr_ready / esr_status / esr_gc / esr_model — ESR-lite surface
 *
 ...
 * fall back to `ctx.sessionQuery` FTS over past sessions when mounted, and to
 * a bounded CJK substring scan over the workspace's own session logs when the
 * query is Chinese (see session-cjk.js).
 */

import { z } from "zod";
import { basename } from "node:path";
import { EngramError, hasCode } from "./store.js";
import { buildEsrStatusView } from "./index-block.js";
import { truncate, shortId, slugId, fmtDate, escapeLt, workspaceKey as wk, uuid } from "./util.js";
import { hasCJK, searchSessionsCJK } from "./session-cjk.js";
...
function memoryLine(m, verbosity = "short", rel, homeWs) {
  // origin marker for cross-workspace hits: [W:<ws>] tells the model the memory
  // came from a different workspace pool (issue #5).
  const origin = homeWs !== void 0 && homeWs !== null && (m._origin ?? m.workspace) !== homeWs
    ? ` [W:${basename(m._origin ?? m.workspace) || String(m._origin ?? m.workspace)}]`
    : "";
  const tags = m.entity !== null ? ` · entity:${m.entity}` : "";
  const relTail = rel
    ? `${rel.supersededBy.has(m.id) ? ` · superseded by ${rel.supersededBy.get(m.id).id.slice(0, 8)}` : ""}${rel.contradictedBy.has(m.id) ? ` · contradicted by ${rel.contradictedBy.get(m.id).map((c) => c.id.slice(0, 8)).join(",")}` : ""}`
    : "";
  if (verbosity === "full") {
    ...
    const relLine = relTail.length > 0 ? relTail.trim() : "";
    const sourceLine = m.source ? ` · source ${m.source}` : "";
    const condLine = m.conditions ? ` · conditions ${m.conditions}` : "";
    return [`# ${m.id} [${m.kind}]${origin}`, escapeLt(m.text), `tags: ${(m.tags ?? []).join(", ")}${tags}`, `signal ${m.signal} · hits ${m.hits ?? 0} · ${fmtDate(m.createdAt)} · session ${m.sessionId}#${m.seq} · ${access}${pathLine}${relLine ? ` · ${relLine}` : ""}${sourceLine}${condLine}`, ""].join("\n");
  }
  const date = fmtDate(m.createdAt ?? m.updatedAt);
  const path = m.filePath ? ` · ${truncate(m.filePath, 48)}` : "";
  return `- ${m.id.slice(0, 8)} [${m.kind}]${origin} ${date} ${escapeLt(truncate(m.text, 200))}${tags}${m.hits > 0 ? ` · ×${m.hits}` : ""}${path}${relTail}`;
}
 ...
 */
function groupKindLines(items, rel, homeWs) {
  ...
  const emit = (kind, group) => {
    ...
    for (const m of group) lines.push(memoryLine(m, "short", rel, homeWs));
  };
...
export function registerTools(ctx, service) {
  return ctx.effect(() => {
    ...
    const disposers = [
      ...
      registerTool({
        name: "engram_store",
        description: "Store one memory (kind, text, optional entity anchor + supersedes/contradicts id). Duplicate text refreshes; auto-capture covers git/file/error — use this for anything missed.",
        parameters: {
          type: "object",
          properties: {
            ...
            supersedes: { type: "string", description: "Optional memory id this new statement replaces (old row stays, demoted in recall)" },
            contradicts: { type: "string", description: "Optional memory id this new statement directly contradicts (both stay, flagged)" },
            source: { type: "string", description: "Optional provenance source class: manual | session | report | upstream | handoff" },
            conditions: { type: "string", description: "Optional trigger/condition under which this memory should be recalled" },
          },
          required: ["text"],
        },
        output: TEXT_OUTPUT,
        execute: async (raw, exec) => {
          const args = parse(z.object({ text: z.string().min(1).max(2000), kind: z.enum(KINDS).optional(), tags: z.array(z.string()).optional(), entity: z.string().optional(), file_path: z.string().optional(), supersedes: z.string().optional(), contradicts: z.string().optional(), source: z.string().optional(), conditions: z.string().optional() }), raw, "engram_store");
          ...
          const { stored, duplicated, revived } = await domain.storeMemory(
            {
              ...
              contradicts: args.contradicts ?? null,
      ...
      registerTool({
        name: "engram_recall",
        description: "Deterministic recall (BM25: exact tag first, recency ties). Literal, not semantic. query=filter, entity=timeline, search_sessions=true adds past-session FTS; limit/max_per_session/group_by_kind tune.",
        parameters: {
          type: "object",
          properties: {
            ...
            max_per_session: { type: "integer", minimum: 1, maximum: 10, description: "Max memories kept per sessionId (default 3; sessions share the cap)" },
            ...
            scope: { type: "string", enum: ["workspace", "global", "auto"], description: "Recall scope override: workspace=strict current-workspace only (default); global=include other workspaces' pools when the local pool is weak/empty (marks [W:<ws>] origin); auto=follow the recallScope config (recommended)" },
          },
        },
        output: TEXT_OUTPUT,
        timeoutMs: 30000,
        execute: async (raw, exec) => {
          const args = parse(z.object({ query: z.string().optional(), entity: z.string().optional(), limit: z.number().int().min(1).max(50).optional(), max_per_session: z.number().int().min(1).max(10).optional(), group_by_kind: z.boolean().optional(), search_sessions: z.boolean().optional(), scope: z.enum(["workspace", "global", "auto"]).optional() }), raw, "engram_recall");
          const caller = callerOf(exec);
          const domain = await requireDomain(service);
          let lines = [];
          let items = [];
          ...
          let zeroHits = false;
          // recallScope resolution: explicit scope wins; else the config knob;
          // default "workspace" preserves the strict per-workspace isolation.
          const cfgScope = service.config.recallScope ?? "workspace";
          const effectiveScope = args.scope === void 0 ? cfgScope : args.scope === "auto" ? cfgScope : args.scope;
          const globalEnabled = effectiveScope === "global";
          if (args.entity !== void 0 && args.entity.trim().length > 0) {
            ...
            items = domain.recall(caller.cwd, args.query, limit, { maxPerSession });
            let cross = false;
            // Opt-in global fallback (issue #5): only when the local pool is
            // weak/no-hit AND the user enabled recallScope=global or passed
            // scope=global. Cross-workspace hits carry a [W:<ws>] origin marker.
            if (globalEnabled && items.length === 0) {
              const extra = domain.recallGlobal(args.query, limit, { maxPerSession, excludeWorkspace: caller.cwd });
              if (extra.length > 0) {
                items = extra;
                cross = true;
              }
            ...
            if (items.length === 0) zeroHits = true;
            const scopeTag = cross ? " · cross-workspace" : "";
            lines.push(zeroHits ? `no active memories match ${JSON.stringify(args.query)} in this workspace` + (globalEnabled ? " (global fallback also empty)" : "") : `# recall: ${JSON.stringify(args.query)} (${items.length}) · ≤${maxPerSession}/session${scopeTag}`);
            const rel = domain.memoryRelations(caller.cwd);
            const homeWs = caller.cwd;
            lines.push(...(args.group_by_kind === false ? items.map((m) => memoryLine(m, "short", rel, homeWs)) : groupKindLines(items, rel, homeWs)));
            await Promise.all(items.map((m) => domain.touchMemory(caller.cwd, m.id).catch(() => null)));
          } else {
          ...
          if (wantSessions) {
            ...
            const extra = await searchSessions(service, caller, q, exec.signal, globalEnabled);
            if (extra.length > 0) {
              lines.push("", "# past sessions (FTS fallback)", ...extra);
            } else if (hasCJK(q)) {
              // The host FTS index is tokenizer-blind to Chinese (unicode61
              // keeps CJK runs as single tokens), so supplement it with a
              // bounded substring scan over this workspace's own sessions.
              const cjk = searchSessionsCJK({ cwd: caller.cwd, query: q });
              if (cjk.length > 0) {
                lines.push("", "# past sessions (CJK substring scan)", ...cjk.map((h, i) => `${i + 1}. session ${h.sessionId} · ${truncate(h.snippet, 160)}`));
              } else if (zeroHits && lines.length === 1) {
                lines[0] += " (no past-session hits either)";
              }
            } else if (zeroHits && lines.length === 1) {
              lines[0] += " (no past-session hits either)";
            }
      ...
      registerTool({
        name: "engram_detail",
        description: "Full record of one memory id: text, tags, entity, signal, hits, session#seq provenance.",
        parameters: {
          type: "object",
          properties: {
        ...
        execute: async (raw, exec) => {
          ...
          return memoryLine(m, "full", domain.memoryRelations(caller.cwd));
        },
      }),
      ...
      registerTool({
        name: "esr_task",
        description: "Create/update an engineering task (draft→active). Done only with evidence — esr_close (artifact+evaluation+memory_ref). id=update; entity=auto-create node + relates_to link.",
        parameters: {
          type: "object",
          properties: {
            ...
            entity: { type: "string", description: "Domain object to hang this task on — auto-creates ent_<slug> node + task --relates_to--> edge" },
          },
          required: ["name"],
        },
        output: TEXT_OUTPUT,
      ...
      registerTool({
        name: "esr_close",
        description: "Close a task via the ESR evidence protocol: artifact (path/url) + evaluation + memory_ref — missing gates keep it active; force:true skips the disk check. Stores a closing memory.",
        parameters: {
          type: "object",
          properties: {
            task_id: { type: "string", description: "Task id from [ESR] or esr_task" },
            artifact: { type: "string", description: "Produced artifact path/url. Resolved against workspace; must exist on disk unless force:true" },
...
${hint}` : ""}`;
      ...
      registerTool({
        name: "esr_link",
        description: "Add a typed relation between entities (depends_on/implements/refines/contradicts…). Feeds entity queries + link count.",
        parameters: {
          type: "object",
          properties: {
      ...
      registerTool({
        name: "esr_dep",
        description: "Add a dependency edge between tasks: blocks (until dep closes), parent-of (child), relates-to (no blocking). Refuses self-deps/cycles.",
        parameters: {
          type: "object",
          properties: {
      ...
      registerTool({
        name: "esr_claim",
        description: "Atomically claim a task (draft→active, assignee+claimedAt). Refuses when another agent holds it unless force:true. Find work with esr_ready.",
        parameters: {
          type: "object",
          properties: {
            task_id: { type: "string", description: "Task id from [ESR] or esr_task" },
      ...
      registerTool({
        name: "esr_unclaim",
        description: "Release a claimed task's assignee to null. Non-holder needs force:true. Inverse of esr_claim.",
        parameters: {
          type: "object",
          properties: {
            task_id: { type: "string", description: "Task id from [ESR] or esr_task" },
      ...
      registerTool({
        name: "esr_status",
        description:
          "Live ESR state (deterministic snapshot + revision) + decision-point hints (promote/root-cause/close/stale/escalate). Pass since_revision to skip unchanged state (short response). Prompt [ESR] is a frozen session-start snapshot — call this for current truth.",
        parameters: {
          type: "object",
          properties: {
            workspace: { type: "string", description: "Override the workspace key (default: current session cwd)" },
            since_revision: { type: "string", description: "Last-seen ESR revision; unchanged state returns a short response" },
          },
        },
        output: TEXT_OUTPUT,
        timeoutMs: 20000,
        execute: async (raw, exec) => {
          const args = parse(z.object({ workspace: z.string().optional(), since_revision: z.string().optional() }), raw, "esr_status");
          const caller = callerOf(exec);
          const domain = await requireDomain(service);
          const ws = wk(args.workspace ?? caller.cwd);
          return buildEsrStatusView(domain, ws, service.config, {
            recorder: service.recorder,
            sessionId: caller.sessionId,
            sinceRevision: args.since_revision,
          });
        },
      }),
      ...
      registerTool({
        name: "esr_node",
        description: "Create/update an entity node — stable symbol for a recurring thing (package/service/doc/repo/person…). Anchors esr_link + memory entity; same name updates; id=ent_<slug>.",
        parameters: {
          type: "object",
          properties: {
...
${hint}` : ""}`;
      ...
      registerTool({
        name: "esr_gc",
        description: "Run memory GC: archives TTL-expired, over-cap low-retention memories, stable tasks past retention, dangling links. Working set protected; archive-only, never hard-deletes. dry_run previews.",
        parameters: {
          type: "object",
          properties: {
            workspace: { type: "string", description: "Override the workspace key (default: current session cwd)" },
      ...
      registerTool({
        name: "esr_model",
        description: "Precomputed mental model (task counts, entity graph, top evidence-grounded beliefs, risks). Zero LLM. mode=brief → one-liner; max_chars trims. Read before scattering recall.",
        parameters: {
          type: "object",
          properties: {
 ...
 *  `global` (recallScope=global) drops the cwd filter so past-session hits from
 *  other workspaces are reachable too — matching the cross-workspace scope. */
async function searchSessions(service, caller, query, signal, global) {
  ...
  try {
    const { items = [] } = await sq.searchSessions(
      {
        query: query.trim(),
        sessionFilters: global ? [] : [{ kind: "cwd", values: [caller.cwd] }],
        limit: 5,
      },
