 ...
 * never steal a [LOOM] index line unless recall has proven it useful.
 */
...
export function renderIndex(store, workspace, cwd, config) {
  ...
  const lines = [];
  lines.push(`[LOOM] workspace: ${label(cwd)} · ${entries.memories} memories · ${tasks.length} task(s) active · ${links} links · ${entries.nodes} node(s)`);
  ...
  lines.push("drill: loom_recall <query> | loom_detail <id> | esr_task (multi-step work → create now) | esr_node (recurring domain object → create now) | esr_link (relate nodes/tasks)");
...
export function escalationHint(store, workspace) {
  ...
  const MEM = ["loom_store", "loom_recall", "loom_detail"];
