...
export function renderIndex(store, workspace, cwd, config) {
  ...
  lines.push("drill: loom_store (user asks to remember) | loom_recall <query> | loom_detail <id> | esr_task / esr_close / esr_link");
...
export function renderEsr(store, workspace, config) {
  ...
  if (active.length === 0 && stable.length === 0) {
    ...
    return "[ESR] no open tasks — for multi-step work create one with esr_task and close it with evidence (esr_close)";
  }
