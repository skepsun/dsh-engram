...
export function renderIndex(store, workspace, cwd, config) {
  ...
  lines.push("drill: use [RECALL] below · engram_detail <id> (full record) · engram_recall <query> (more) · esr_task/esr_node/esr_link (work)");
...
/** Text of the session's first user message (the opening instruction of the
 *  conversation). Walk `session.events` for the first `user/message` event and
 *  join its `data.content` text blocks; fall back to the first text-bearing
 *  event ("goal"/reminder start) guardedly. Returns "" when there is none.
 *  Pure, deterministic — the auto-recall (B) keys its query off this. */
export function firstUserMessageText(session) {
  ...
  // Fallback: no user/message yet — first event carrying text (goal-driven,
  // headless or reminder-seeded starts) still gives the recall a key.
  for (const event of events) {
...
/**
 * Host-side session-start auto-recall (方案 B): at the first assembly of a new
 * session, deterministically key a bounded BM25 recall off the session's first
 * user message and inject the top hits as a `[RECALL]` block — so relevant
 * memory reaches the model even when it never calls `engram_recall` on its own
 * (the observed real-world gap). Pure rule, zero LLM, no `hits` bumps (a pure
 * read). Falls back to the newest high-signal memories when the query is empty
 * and `allowRecencyFallback` is on. Returns "" when nothing useful to show.
 */
export function renderStartRecall(store, workspace, config = {}, opts = {}) {
  if (store === null || typeof store?.recall !== "function") return "";
  const limit = Math.max(1, Math.min(8, Number.isFinite(config.autoRecallLimit) ? Math.trunc(config.autoRecallLimit) : 3));
  const maxChars = Number.isFinite(config.autoRecallMaxChars) && config.autoRecallMaxChars > 0 ? Math.trunc(config.autoRecallMaxChars) : 700;
  const rawQuery = String(opts.query ?? "").trim().slice(0, 300);
  const query = rawQuery.length > 0 ? rawQuery : (opts.allowRecencyFallback ?? true) ? null : "";
  const items = query === null
    ? (store.listMemories?.(workspace, limit) ?? []).slice(0, limit)
    : store.recall(workspace, query, limit, { maxPerSession: limit });
  const hits = Array.isArray(items) ? items.filter((m) => m !== null && m !== void 0) : [];
  // "Few and precise" (agentmemory 候选①): never let a superseded stale row —
  // which `recall` demotes to the tail of an unbounded list — occupy one of the
  // ≤ N auto-injected slots. Same policy as the [ENGRAM] index. The stale row
  // stays re-fetchable via engram_detail, just not auto-pushed.
  const { supersededBy } = store.memoryRelations?.(workspace) ?? { supersededBy: new Map() };
  const fresh = hits.filter((m) => !supersededBy.has(m.id));
  if (fresh.length === 0) return "";

  const lines = [];
  const source = query === null ? "newest memory" : "recall";
  lines.push(`[RECALL] ${source} · ${fresh.length} hit(s)${query ? ` · first msg: ${escapeLt(truncate(query, 40))}` : " · no user text yet"}`);
  for (const m of fresh) {
    const px = prefixFor(m.kind);
    const mark = isProvenProcedure(m, config) ? `${px}✓` : px;
    const date = fmtDate(m.createdAt ?? m.updatedAt);
    const text = truncate(String(m.text).replace(/\s*\n+\s*/g, " ").trim(), 140);
    lines.push(`- [${mark}] ${date} ${escapeLt(text)} #${shortId(m.id)}${(m.hits ?? 0) > 0 ? ` ×${m.hits}` : ""}`);
  }
  lines.push("context: use above · engram_detail <id> · engram_recall <query>");
  return enforceStartRecallCap(lines, maxChars);
}

/** Keep the [RECALL] header + context tail, drop hits until under budget;
 *  hard-truncate with an ellipsis when even the shell exceeds the cap. */
function enforceStartRecallCap(lines, maxChars) {
  let joined = lines.join("\n");
  while (joined.length > maxChars && lines.length > 2) {
    lines.splice(lines.length - 2, 1);
    joined = lines.join("\n");
  ...
  if (joined.length > maxChars) {
    joined = `${joined.slice(0, Math.max(1, maxChars - 1))}…`;
  }
  return joined;
}
